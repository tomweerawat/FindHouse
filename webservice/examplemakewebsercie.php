...
function dbcon(){
 ...
 while ( $row = $query->fetch_assoc()) {
  ...
  $result2['location'] = $row['house']." ".$row['number']
                          ." ".$row['road']." ".$row['subdistrict']
                          ." ".$row['district']." ".$row['province']." ".$row['zipcode'];
  $result2['status'] = $row['proptype'];
  $result2['price'] = $row['price'];
  $result2['description'] = $row['detail'];
  $result2['activation'] = $row['activation'];
  $result2['img1'] = "http://10.255.21.174:8181/FindHouse/".$row['img1'];
  $result2['img2'] = "http://10.255.21.174:8181/FindHouse/".$row['img2'];
  $result2['img3'] = "http://10.255.21.174:8181/FindHouse/".$row['img3'];
  $result2['img4'] = "http://10.255.21.174:8181/FindHouse/".$row['img4'];
  $result2['img5'] = "http://10.255.21.174:8181/FindHouse/".$row['img5'];
